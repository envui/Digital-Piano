...
{
  while(units--){
    // split 0.1s into 10 chunks so switches feel immediate
...
void Audio_Init(void){
  S1_Audio_Init();
  S1_Audio_Select(); // SysTick task = square-wave toggle
}
...
void Audio_PlayToneIndex(uint8_t toneIndex){
  ...
  S1_Audio_PlayToneIndex(toneIndex);
}
  ...
  while(*modePtr == 1){
    ...
    while(mysong[songStart][i].delay){
      ...
      if(*songSelPtr != songStart){
        Audio_Stop();
        break; // restart immediately with new song
      }
      ...
      if(idx == PAUSE){
        ...
        // transpose using Tone_Tab layout: 0..6 per octave
    ...
    // pause between repeats, still abortable
    if(*modePtr != 1) return;
